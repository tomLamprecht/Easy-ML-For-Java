...
public class PoolingLayer {
    ...
    private void insertPooledValuesInto2dArray(Matrix input, double[][] result) {
        for (int row = 0; row < input.getNumRows(); row+=rowSize) {
            for (int col = 0; col < input.getNumCols(); col+=colSize) {
    ...
    private double[][] createEmptyPooling2dArray(Matrix input) {
        int resultRows = (int) Math.ceil( input.getNumRows() * 1d / rowSize);
        int resultCols = (int) Math.ceil(input.getNumCols() * 1d / colSize);
